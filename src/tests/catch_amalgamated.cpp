...
ScopedMessage::ScopedMessage(ScopedMessage && old) noexcept : m_info(std::move(old.m_info))
{
  old.m_moved = true;
}
...
public:
  explicit TestGroup(IStreamingReporterPtr && reporter, Config const * config)
    : m_reporter(reporter.get()), m_config{config}, m_context{config, std::move(reporter)}
  {
...
Detail::unique_ptr<TestCaseInfo> makeTestCaseInfo(std::string const &    _className,
  ...
  , lineInfo(_lineInfo)
{
  ...
  tags.erase(
    std::unique(begin(tags), end(tags), [](Tag lhs, Tag rhs) { return lhs.lowerCased == rhs.lowerCased; }),
    end(tags));
}
...
bool TestCaseHandle::operator<(TestCaseHandle const & rhs) const { return m_info->name < rhs.m_info->name; }
...
{
  return std::find_if(begin(testCase.tags), end(testCase.tags), [&](Tag const & tag) {
...
{
  return std::any_of(
    m_filters.begin(), m_filters.end(), [&](Filter const & f) { return f.matches(testCase); });
}
                                       ...
                                       IConfig const &                          config)
{
  ...
  for(auto const & desc : descriptions)
  {
    ...
    {
      Catch::cout() << TextFlow::Column(desc.name).indent(2).width(5 + maxNameLen) << '\n';
    }
    else
    {
      Catch::cout() << TextFlow::Column(desc.name + ":").indent(2).width(5 + maxNameLen) +
...
void IStreamingReporter::listTests(std::vector<TestCaseHandle> const & tests, IConfig const & config)
{
  ...
  for(auto const & test : tests)
  {
    ...
    {
      Catch::cout() << TextFlow::Column(Catch::Detail::stringify(testCaseInfo.lineInfo)).indent(4)
                    << std::endl;
    }
    ...
    if(isMatch(token.token))
    {
      if(m_ref->isFlag())
      {
        ...
        if(result.value() == ParseResultType::ShortCircuitAll)
          return Detail::InternalParseResult::ok(Detail::ParseState(result.value(), remainingTokens));
      }
      else
      {
        auto valueRef = static_cast<Detail::BoundValueRefBase *>(m_ref.get());
...
Detail::InternalParseResult ExeName::parse(std::string const &, Detail::TokenStream const & tokens) const
{
  return Detail::InternalParseResult::ok(Detail::ParseState(ParseResultType::NoMatch, tokens));
}
                                          ...
                                          Detail::TokenStream const & tokens) const
{
  ...
  while(result.value().remainingTokens())
  {
    ...
    if(result.value().type() == ParseResultType::ShortCircuitAll)
      return result;
    if(!tokenParsed)
      return Detail::InternalParseResult::runtimeError("Unrecognised token: " +
...
private:
  void setColour(const char * _escapeCode)
  {
    ...
    getCurrentContext().getConfig()->stream() << '\033' << _escapeCode << std::flush;
  }
};
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker
{
  ...
  void close() override
  {
    ...
    const bool should_wait_for_child =
      !m_children.empty() &&
      std::find_if(m_children.begin(), m_children.end(), [](TestCaseTracking::ITrackerPtr tracker) {
...
Section::Section(SectionInfo && info)
  : m_info(std::move(info)), m_sectionIncluded(getResultCapture().sectionStarted(m_info, m_assertions))
{
...
std::vector<TestCaseHandle> sortTests(IConfig const &                     config,
...
std::vector<TestCaseHandle> filterTests(std::vector<TestCaseHandle> const & testCases,
...
void ITracker::addChild(ITrackerPtr const & child) { m_children.push_back(child); }
...
ITracker & TrackerContext::startRun()
{
  ...
  m_rootTracker =
    std::make_shared<SectionTracker>(NameAndLocation("{root}"s, CATCH_INTERNAL_LINEINFO), *this, nullptr);
...
void TrackerBase::close()
{
  ...
  case ExecutingChildren:
    if(std::all_of(
         m_children.begin(), m_children.end(), [](ITrackerPtr const & t) { return t->isComplete(); }))
      m_runState = CompletedSuccessfully;
    break;
                 ...
                 NameAndTags const &              nameAndTags) noexcept
{
  CATCH_TRY
  {
    getMutableRegistryHub().registerTest(
      makeTestCaseInfo(extractClassName(classOrMethod), nameAndTags, lineInfo), std::move(invoker));
  }
  CATCH_CATCH_ALL
  {
    // Do not throw when constructing global objects, instead register the exception to be processed later
...
{
  return static_cast<XmlFormatting>(static_cast<std::underlying_type_t<XmlFormatting>>(lhs) |
...
{
  return m_caseSensitivity == CaseSensitive::Yes ? StringRef() : " (case insensitive)"_sr;
}
...
void EventListenerBase::listReporters(std::vector<ReporterDescription> const &, IConfig const &) {}
void EventListenerBase::listTests(std::vector<TestCaseHandle> const &, IConfig const &) {}
...
void printTotals(std::ostream & out, const Totals & totals)
{
  ...
  else if(totals.testCases.failed == totals.testCases.total())
  {
    ...
    const StringRef qualify_assertions_failed = totals.assertions.failed == totals.assertions.total() ?
...
void ConsoleReporter::benchmarkPreparing(std::string const & name)
{
  ...
  auto nameCol =
    TextFlow::Column(name).width(static_cast<std::size_t>(m_tablePrinter->columnInfos()[0].width - 2));
...
struct BySectionInfo
{
  ...
  bool operator()(std::shared_ptr<CumulativeReporterBase::SectionNode> const & node) const
  {
...
void StreamingReporterBase::testGroupStarting(GroupInfo const & _groupInfo) { currentGroupInfo = _groupInfo; }

void StreamingReporterBase::testGroupEnded(TestGroupStats const &) { currentGroupInfo.reset(); }
  ...
  if(!result.isOk())
  {
    ...
    if(currentTestCaseInfo->okToFail())
    {
      ...
      stream << "##teamcity[testIgnored"
             << " name='" << escape(currentTestCaseInfo->name) << '\'' << " message='" << escape(msg.str())
             << '\'' << "]\n";
    }
    else
    {
...
void TeamCityReporter::testCaseEnded(TestCaseStats const & testCaseStats)
{
  ...
  if(!testCaseStats.stdOut.empty())
    stream << "##teamcity[testStdOut name='" << escape(testCaseInfo.name) << "' out='"
           << escape(testCaseStats.stdOut) << "']\n";
  if(!testCaseStats.stdErr.empty())
    stream << "##teamcity[testStdErr name='" << escape(testCaseInfo.name) << "' out='"
           << escape(testCaseStats.stdErr) << "']\n";
  stream << "##teamcity[testFinished name='" << escape(testCaseInfo.name) << "' duration='"
